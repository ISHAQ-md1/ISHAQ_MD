...
const {
  ...
  const ownerNumber = ['923307092214']
  ...
  async function connectToWA() {
      ...
      // Log loaded commands
      console.log(`[🔰] Command system loaded, ${events.commands.length} commands registered.`);
...
⏰ *Started at:* ${new Date().toLocaleString()}`;
                  ...
                  await conn.sendMessage(botJid, { 
                      image: { url: config.MENU_IMAGE_URL || 'https://files.catbox.moe/f1ygtp.jpg' }, 
                      caption: upMessage,
                      contextInfo: {
                          forwardingScore: 999,
                          isForwarded: true,
      ...
      conn.ev.on('messages.upsert', async(mek) => {
        try {
            ...
            if (isCmd) {
                ...
                // Fallback ping command if no plugin found
                if (!cmd && cmdName === 'ping') {
                ...
                if (cmd) {
                    ...
                    // Optional: Uncomment to notify unknown commands
                    // reply(`❌ Unknown command: ${cmdName}\nType .menu for available commands.`);
                }
            }
            
            // Additional event handlers for non-command triggers
